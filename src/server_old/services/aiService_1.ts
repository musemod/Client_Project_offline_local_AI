...
// import { TgenerateSchemaDescription } from "../sql_db/schemas-helper";
...
private extractSQLFromResponse(sql: string): string {
  // Strategy 1: Find SQL between triple backticks
  const backtickMatch = sql.match(/```sql\n([\s\S]*?)\n```|```\n([\s\S]*?)\n```/i);
  if (backtickMatch) {
    return (backtickMatch[1] || backtickMatch[2]).trim();
  }
  
  // Strategy 2: Look for SELECT ... ; pattern
  const selectMatch = sql.match(/(SELECT[\s\S]*?;)/i);
  if (selectMatch) {
    return selectMatch[1].trim();
  }
  
  // Strategy 3: Just take everything and hope
  return sql.trim();
}

private cleanSQL(sql: string): string {
  // Remove any explanatory text before/after
  const extracted = this.extractSQLFromResponse(sql);
  
  // Basic quote normalization
  let cleaned = extracted.replace(/""/g, '"');
...
// Remove all other post-processing functions (fixDoubleQuotes, fixUnionAll, extractAliases, ensureQuotedIdentifiers)
...
private buildSystemPrompt(schemaDescription: string): string {
  return `Generate PostgreSQL for this schema. Return ONLY the SQL.

Schema:
${schemaDescription}

Examples:
Q: who are the technical delivery managers and what controls are they responsible for
SQL: SELECT t."firstName", t."lastName", c."short", c."long" FROM "allTeams" t JOIN "allTrustControls" c ON t."category" = c."category" WHERE t."role" ILIKE '%Technical Delivery Manager%' AND t."isActive" = true;

Q: how many controls are in each category
SQL: SELECT "category", COUNT(*) as control_count FROM "allTrustControls" GROUP BY "category";

Q: which team members are responsible for cloud security
SQL: SELECT * FROM "allTeams" WHERE "category" ILIKE 'Cloud Security' AND "isActive" = true;`;
}
  ...
  async textToSQL(options: TextToSQLOptions): Promise<string> {
    ...
    // const endpoint = 'http://ollama:11434/v1/completions';
    ...
    console.log('SCHEMA BEING SENT TO MODEL:', schemaDescription);

    console.log('FULL REQUEST TO MODEL:', JSON.stringify({
  model: this.modelName,
  messages: [
    { role: 'system', content: this.buildSystemPrompt(schemaDescription) },
    { role: 'user', content: `Generate ONLY the SQL query for: ${prompt}` }
  ]
}, null, 2));
...
const response = await fetch(endpoint, {
  ...
  body: JSON.stringify({
    ...
    max_tokens: 200,
    stream: false,
    // Stop at any explanatory text
    stop: [
      '\n\n',           // Double newline often starts explanation
      'Answer:',        // Common chat marker
      'Explanation:',   // Common in text2sql models
      '```\n\n',        // End of code block + newline
      'The SQL query',  // Start of explanation
      'This query',     // Start of explanation
      'Note:'           // Start of note
    ]
  })
});
